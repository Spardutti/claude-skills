...
- Publish flow: bump version → **`bash scripts/preflight.sh`** → commit → push to `main` → **push the tag** `v<version>`, which is what publishes → **GitHub release**.
- **The tag publishes, not your laptop.** `.github/workflows/publish.yml` runs on a pushed `v*` tag and publishes over OIDC (npm Trusted Publishing), so there is no token in the repo and no 2FA prompt. It refuses a tag that disagrees with `cli/package.json`. Preflight still runs locally first — it surveys real repositories on the machine and cannot run in CI. `npm publish ./cli --access public` by hand still works if CI is down.
- **A skill that names a version declares it in frontmatter** as `tracks: react@19.2, react-hook-form@7.87` (add ` (pypi)` for a Python package). `scripts/check-freshness.mjs` asks the registries and preflight fails on a MAJOR behind — a skill teaching a superseded major is worse than one that names no version at all. A minor behind is reported, not fatal.
...
- **Assert the argument list, not the verdict.** A stub is written from the same belief as the code, so it agrees with the bug and the verdict still comes out green. The gate handed Stryker a repeated `--mutate` for four releases — a flag Stryker's CLI overrides rather than appends, so exactly one file per run was ever mutated — and every self-test passed the whole time, because they only asked whether the gate said green. `--incremental` was the same shape before it. When the gate builds a command for a real tool, stub that tool to log each argument and compare the whole list; a flag that repeats, a flag that comes back, or a value that lost a file all fail there and nowhere else.
