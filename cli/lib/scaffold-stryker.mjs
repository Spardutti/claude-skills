...
const IGNORER_VERSION = "v2";

// Exported and then serialised into the generated ignorer below, rather than
// written twice. A copy in the template and a copy under test would agree with
// each other and disagree with the shipped file.
export function inDataTable(path) {
  let found = false;
  for (let p = path; p; p = p.parentPath) {
    if (p.isFunction()) return false;
    if (!found && p.isVariableDeclarator()) {
      const init = p.node.init;
      if (!init) return false;
      if (init.type !== "ArrayExpression" && init.type !== "ObjectExpression") return false;
      found = true;
    }
  }
  return found;
...
// A string in a module-level array or object literal is copy: a heading, a
// label, a term row. Mutating it produces 'Pizzas' -> 'Stryker was here!',
// which can only be killed by asserting the table against a copy of itself —
// the tautology the testing skill exists to stop. One table of 11 headings and
// 16 rows produced 57 such survivors and a fifteen-minute gate.
//
// Any function between the string and the top disqualifies it: that is code
// building a value, not a table of content. A non-string mutant in the same
// table is left alone, because a number or a flag in a config object is
// behaviour even when it sits in data.
${inDataTable.toString()}
  ...
  declareValuePlugin(PluginKind.Ignore, "tailwind-classnames", {
    shouldIgnore(path) {
      ...
      if (path.find((p) => p.isCallExpression() && p.node.callee.name === "cn"))
        return "styling, not behaviour";
      if (path.isStringLiteral() && inDataTable(path))
        return "copy in a data table, not behaviour";
    },
  }),
];
`;
