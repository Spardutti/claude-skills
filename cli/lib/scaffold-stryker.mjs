...
// ignoreStatic is the whole reason a first run is survivable. A string in a
// module-level array or object is a static mutant, evaluated once at file load,
// and Stryker's docs say testing one "often requires running all tests". A table
// of 11 headings and 16 term rows produced 57 survivors and a gate that timed
// out past 600s; with this set the same gate ran in 1m33s and the survivors were
// gone without a single test written for them. It requires perTest coverage
// analysis, which is why the two belong together.
//
// A shipped attempt to solve this in the ignorer — skip string mutants inside a
// module-level array or object literal — was exactly the definition of a static
// mutant, so it did the same job worse. It is gone; this option replaces it.
const CONFIG = {
...
import { IGNORER, IGNORER_VERSION } from "./stryker-ignorer.mjs";
...
export async function scaffoldStryker(projectDir, rel) {
  ...
  if (await exists(configPath)) {
    ...
    if (cfg === null) {
      ...
      const needsIgnorer = !(cfg.ignorers ?? []).includes("tailwind-classnames");

      // ignoreStatic is only correct under perTest coverage analysis. Absent, we
      // set both. Already perTest, we add the flag. Anything else was a
      // deliberate choice and stays — silently switching it would change which
      // mutants the project reports, which is not ours to decide.
      const coverage = cfg.coverageAnalysis;
      const canSetStatic = coverage === undefined || coverage === "perTest";
      const needsStatic = cfg.ignoreStatic !== true && canSetStatic;

      if (!needsIgnorer && !needsStatic) {
  ...
  // Testing the fresh install here would have missed the whole point: a project
  // that already had the v1 ignorer would have kept it forever and never gained
  // the data-table rule, which is the rule this release exists for.
  const ignorerPath = join(dir, "stryker-classname-ignorer.mjs");
  let existingIgnorer = null;
  try {
    existingIgnorer = await readFile(ignorerPath, "utf-8");
  } catch {
    // not scaffolded yet
  }
  if (existingIgnorer === null) {
    await writeFile(ignorerPath, IGNORER);
    wrote.push(`${at}stryker-classname-ignorer.mjs`);
  } else if (existingIgnorer.includes(`ignorer ${IGNORER_VERSION}`)) {
    kept.push(`${at}stryker-classname-ignorer.mjs`);
  } else {
    await writeFile(ignorerPath, IGNORER);
    patched.push(`${at}stryker-classname-ignorer.mjs`);
  }
